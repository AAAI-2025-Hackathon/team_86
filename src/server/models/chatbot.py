...
from server.lib.constants import OLLAMA_URL, DEFAULT_GENAI_MODEL
...
class Chatbot:
    ...
    def __call__(self, webpage_text: str, category: WebsiteCategory, model_name: ModelName = DEFAULT_GENAI_MODEL, conv: Conversation = []) -> str:
        self._parse_conversation(conv)
        return self.chat(conv, webpage_text, category, model_name)
    ...
    async def chat(self, conv: Conversation, webpage_text: str, category: WebsiteCategory, model_name: ModelName) -> str:
        ...
        try:
            ...
            response = await format_llm_response(response)
            self._reset_history()
            return response
        except:
